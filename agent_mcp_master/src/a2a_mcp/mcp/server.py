...
from a2a_mcp.common.nih_accession_lookup import *
from a2a_mcp.common.gene_accession_lookup import SimpleAccessionLookup
...
def serve(host, port, transport):  # noqa: PLR0915
    ...
    def find_agent(query: str) -> str:
        ...
        logger.info(df.iloc[best_match_index]['agent_card'])
        return df.iloc[best_match_index]['agent_card']

    @mcp.tool(
        name='get_gene_info',
        description='Get gene information for a given gene name.',
    )
    def get_gene_info(gene_name: str) -> dict:
        """ "name": "get_gene_info",
        "description": "Retrieves the most up-to-date gene information from NIH.
        This tool should be used when a user asks for gene lookup.",
        
        Args:
            query: The natural language query string used to search for a
                   relevant agent.
        Returns:
            The json with gene information.
        """
        try:
            accession_number =""
            accesion_lookup = SimpleAccessionLookup()
            accession_number = accesion_lookup.get_accessions(gene_name)
            logger.info(f'Looking up accession number for : {gene_name}: {accession_number}')

            if accession_number is None or len(accession_number) == 0:
                return {f'Unable to identify the Accession Number for gene: {gene_name}'}
    ...
    def design_guide_rna(gene_name: str) -> dict:
        """Design Guide RNA for a given Gene name.

        This function takes a gene name, and designs guide RNA.

        Returns:
            The json with information .
        """
        logger.info(f'Designing Guide RNA for : {gene_name}')
        return {'guide_rna': "TTTGA"}

    @mcp.tool(
        name='off_target_validator',
        description='Validate off-target impact',
    )
    def off_target_validator(gene_name: str) -> dict:
        """Validates off-traget impact for a given Gene name.
