...
async def gene_lookup(session: ClientSession) -> CallToolResult:
    """Calls the 'get_gene_info' tool on the connected MCP server.
    ...
    Returns:
        The result of the tool call.
    """
    logger.info("Calling 'get_gene_info' tool'")
    return await session.call_tool(
        name='get_gene_info',
        arguments={
            'gene_name': 'SERPINA1'
...
async def main(host, port, transport, query, resource, tool):
    ...
    async with init_session(host, port, transport) as session:
        ...
        if tool:
            if tool == 'gene_lookup':
                results = await gene_lookup(session)
...
@click.option('--tool_name', type=click.Choice(['find_agent', 'gene_lookup', 'design_guide_rna', 'off_target_validation']),
              help='Tool to execute: gene_lookup, design_guide_rna, or off_target_validation')
