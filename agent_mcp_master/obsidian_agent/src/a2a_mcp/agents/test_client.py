...
async def main() -> None:
    ...
    async with httpx.AsyncClient() as httpx_client:
        ...
        try:
            ...
            logger.error(
                f'Critical error fetching public agent card: {e}', exc_info=False
            )
            raise RuntimeError(
        ...
        send_message_payload: dict[str, Any] = {
            'message': {
                'role': 'user',
                'parts': [
                    {'kind': 'text', 'text': "Append the following content in Obsidian file named welcome.md.The content is: hello rama"}
                ],
                'messageId': uuid4().hex
            },
        }
