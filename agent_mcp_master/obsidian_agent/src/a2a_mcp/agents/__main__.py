...
def get_agent(agent_card: AgentCard):
    ...
    try:
        if agent_card.name == 'Experiment Planning Agent':
            return ObsidianAgent(
                agent_name='ExperimentPlanningAgent',
                description='Helps with planning CAS-13 CRISPR RNA editing experiments',
                instructions=prompts.EXPERMIENT_PLANNING_COT_INSTRUCTIONS,
            )
        else if agent_card.name == 'Grna Planning Agent':
            return ObsidianAgent(
                agent_name='GrnaPlanningAgent',
                description='Helps with planning CAS-13 CRISPR Grna',
                instructions=prompts.EXPERMIENT_PLANNING_COT_INSTRUCTIONS,
            )
        else if agent_card.name == 'Experiment Planning Agent':
            return ObsidianAgent(
                agent_name='ExperimentPlanningAgent',
                description='Helps with planning CAS-13 CRISPR RNA editing experiments',
                instructions=prompts.EXPERMIENT_PLANNING_COT_INSTRUCTIONS,
...
@click.option('--port', 'port', default=27200)
@click.option('--agent-card')
